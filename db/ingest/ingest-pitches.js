...
const filePath = process.argv[2] || path.resolve("./templates/pitch_opportunities.csv");
...
for (let i = 0; i < records.length; i += batchSize) {
  ...
  const { error } = await supabase.from("pitch_opportunities").insert(batch);
  if (error) {
...
console.log("Pitch ingestion complete.");
